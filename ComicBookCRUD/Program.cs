...
namespace ComicBookLibraryManager
{
    class Program
    {
        ...
        private static void ListComicBook(int comicBookId)
        {
            ...
            ConsoleHelper.OutputLine("Publishing House: ");
            
            /*foreach (ComicBookPublishingHouse publish in comicBook.PublishingHouses)
            {
                if (string.IsNullOrEmpty(publish.PublishingHouse.Name)) 
                    Console.WriteLine("UNPUBLISHED");
                
                Console.Write("{0} - {1},", publish.PublishingHouse.Name, publish.PublishingHouse.Country);
            }*/


            var selected = from pubHouse in comicBook.PublishingHouses 
                select pubHouse;
            
            if (selected.Any()) // selected.Any() ....
            {
                // verif bef
                foreach (var pub in comicBook.PublishingHouses)
                {
                    Console.Write(" {0} - {1},", pub.PublishingHouse.Name, pub.PublishingHouse.Country);
                }
            }
            else Console.WriteLine("UNPUBLISHED");

            
            
            
            // if (comicBook.PublishingHouses.Any())
            // {
            //     var comicBookHouses = comicBook.PublishingHouses.ToArray(); 
            //     for (var i = 0; i < comicBookHouses.Length; i++)
            //     {
            //         Console.Write(" {0} - {1}", comicBookHouses[i].PublishingHouse.Name, comicBookHouses[i].PublishingHouse.Country);
            //         
            //         //comicBookHouses.Length < i ? Console.Write(",") : Console.Write(".");
            //     }
            //Console.WriteLine(string.Join(",", comicBook.PublishingHouses.Select(c => c.PublishingHouse.Name)) + ".");
            // }
            // else
            // {
            //     Console.WriteLine("Unpublished");
        ...
        // private static string PrepareMessage(int index, string publishHouseName)
        // {
        //     int total 
        // } 
        ...
        private static void ListComicBookProperties(ComicBook comicBook)
        {
            ...
            ConsoleHelper.OutputLine("6) Publishing Houses: {0}", comicBook.PublishingHouses);
        }
    }
}
